...
# Trigger test run
TEST_RUN_ID="$( \
curl -s -X POST -G "$BASE_API_URL/integrations/jenkins/3/trigger-test-run" \
  -d 'token='$AUTH_TOKEN''\
...
while [ "$STATUS" = 'Pending' ]; do
  ...
  STATUS="$( \
    curl -s "$BASE_API_URL/integrations/jenkins/3/test-run-status?token=$AUTH_TOKEN&testRunId=$TEST_RUN_ID" \
      | jq -r '.status' \
  )"
